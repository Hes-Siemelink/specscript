...
**CRITICAL: Always follow spec-first development. See "Adding New SpecScript Commands: Complete Development Process"
below.**
...
3. **Use `localhost:2525` endpoints** - sample-server.spec.yaml runs automatically during tests providing real HTTP
   endpoints
...
7. **Use sentence case for section titles** - e.g., "Reading temp files created in Markdown", not "Reading Temp Files
   Created In Markdown"
8. **Multiple commands require `---` separators** - YAML doesn't allow duplicate keys, so use `---` between commands or
