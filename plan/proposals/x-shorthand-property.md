# Proposal: `x-shorthand-property` — a scalar shorthand for one object property
...
- **Name:** `x-shorthand-property`. Signals "there is some fancy footwork here," is type-agnostic, uses JSON Schema's
...
- **Validation:** a malformed `x-shorthand-property` fails at load time (see Resolved).
...
## 2. Schema convention: `type: [object, string]` + `x-shorthand-property`
...
`x-shorthand-property` lives wherever the property set lives — for `GET`, that means on the `HttpParameters` property set
...
The shorthand property **already works on the CLI today** with no new code: for `x-shorthand-property: name`,
...
Decision: **rely solely on `--<shorthand-property>` — the simplest thing that already works.** No positional, no
generic `--input` alias.
...
clients call with `{ name: "Hes" }` and ignore the `x-shorthand-property` hint. This is the payoff of the object route
...
| Command | Scalar shorthand | `x-shorthand-property` |
|---|---|---|
...
`x-shorthand-property` and produce `{ <property>: <scalar> }` **before** it becomes `INPUT_VARIABLE`, so
...
- **`InputSchema`** (`commands/scriptinfo/InputSchema.kt`): parse + validate `x-shorthand-property`; expose the name.
  Runtime population is unchanged — input is already an object by the time `populateInputVariables` runs.
- **Built-in shorthand commands**: schema/doc change only (`x-shorthand-property` + `type: [object, string]` + table
  row). They already hand-implement `ValueHandler`; no runtime change.
...
- **Validation timing:** fail at **load** time. A bad `x-shorthand-property` (missing property / non-scalar) is validated
