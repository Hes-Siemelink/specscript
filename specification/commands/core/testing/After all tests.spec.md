...
`After all tests`, like `Tests` and `Before all tests`, is only executed in test mode (`spec --test`). In normal execution
it is ignored.
...
Place `After all tests` after `Tests`. The nested commands run once after the last test completes. The teardown
