...
`Before all tests` runs commands once before all tests in a file. The setup commands are nested inside the
`Before all tests`
block, similar to the `Do` command.

`Before all tests`, like `Tests` and `After all tests`, is only executed in test mode (`spec --test`). In normal
...
Place `Before all tests` before `Tests`. The nested commands run once before the first test. The setup shares the same
context as the tests, so variables and state carry forward.
